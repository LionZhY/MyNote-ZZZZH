...
public:
    vector<vector<int>> threeSum(vector<int>& nums) {
        ...
        for (int i = 0; i < nums.size(); i++)
        {
            ...
            // 去重 a：如果i和i-1元素相同，说明要遍历组合的三数之和在nums[i-1]的时候被组合过了，跳过
            if (i > 0 && nums[i] == nums[i - 1])    continue;

            // a = nums[i]  b = nums[left]  c = nums[right]；sum = a + b + c
            // 固定i，移动left right
            int left = i + 1;
            int right = nums.size() - 1;
            while(left < right) 
            {
                ...
                if (sum > 0) 		right--; // 三数之和 > 0，right向左移动，让和变小
                else if (sum < 0) 	left++;  // 三数之和 < 0，left向右移动，让和变大
                else // 三数之和 = 0，找到一个三元组
                {
                    // 收集当前三元组
...
- sort —— O(n log n)
- 外层`for(i)` O(N)  ×  内层 `while(left < right)` O(N)   =  **$O(N^2)$**
- 整体 $O(N^2+NlogN)=O(N^2)$
...
![image-20251015011241927](./pic/image-20251015011241927.png)
...
public:
    int removeDuplicates(vector<int>& nums) {
        ...
        {
            if (nums[fast] != nums[slow]) // 遇到不相等的再移动slow，fast向前覆盖给slow
            {
                slow++;
                nums[slow] = nums[fast];
            }
        ...
        return slow + 1; // slow 是最后不同数字的下标，个数是 slow + 1
...
## 80 [删除有序数组中的重复项II](https://leetcode.cn/problems/remove-duplicates-from-sorted-array-ii/)![image-20251015011444483](./pic/image-20251015011444483.png)

![image-20251015011523710](./pic/image-20251015011523710.png)
...
public:
    int removeDuplicates(vector<int>& nums) {
        ...
        int slow = 2; // 当前要放元素的位置
        int fast = 2;

        // 每个元素只允许放2个，这2个位置就是 slow - 1, slow - 2
        for (fast = 2; fast < nums.size(); fast++)
        {
            if (nums[slow - 2] != nums[fast]) // slow-2 是上上次放了元素的位置
            {
                nums[slow] = nums[fast];
                slow++;
            }
...
![image-20251009173316570](./pic/image-20251009173316570.png)

![image-20251009173401367](./pic/image-20251009173401367.png)
...
public:
    ...
    int pop() {
        ...
        while (size--)
        {
            que.push(que.front());// 将que头部元素（除了最后一个）重新添加到队尾
            que.pop();
        }

        int result = que.front(); // 现在的头部就是原来的最后一个了
        que.pop();
        return result;
    }
    
    int top() {
        return que.back(); // 直接返回队尾
    }
...
<img src="pic/image-20250721110456300.png" alt="image-20250721110456300" style="zoom:50%;" />



[灵神](https://leetcode.cn/problems/construct-binary-tree-from-inorder-and-postorder-traversal/solutions/2647794/tu-jie-cong-on2-dao-onpythonjavacgojsrus-w8ny)  推荐
...
public:
    TreeNode* buildTree(vector<int>& inorder, vector<int>& postorder) {
        ...
        // 找到左子树的大小 （postorder.back() 就是 root）
        int left_size = ranges::find(inorder, postorder.back()) - inorder.begin();

        // 分割中序
        vector<int> inl(inorder.begin(), inorder.begin() + left_size);  // [9]
        vector<int> inr(inorder.begin() + left_size + 1, inorder.end());// [15 20 7]

        // 分割后序
        vector<int> postl(postorder.begin(), postorder.begin() + left_size);// [9]
        vector<int> postr(postorder.begin() + left_size, postorder.end() - 1);//[15 7 20]

        // 递归左右
        TreeNode* left = buildTree(inl, postl);
        TreeNode* right = buildTree(inr, postr);

        // 返回新树
        return new TreeNode(postorder.back(), left, right); // new (root->val, left, right)
...
public:
    ...
    TreeNode* traversal(vector<int>& inorder, vector<int>& postorder)
    {
        ...
        int delimiterIndex = 0; // 找中序数组的切割下标
...
<img src="pic/image-20250721112428321.png" alt="image-20250721112428321" style="zoom:50%;" />
...
public:
    int diameterOfBinaryTree(TreeNode* root) {
        ...
        auto dfs = [&](this auto&& dfs, TreeNode* node) ->int {
            ...
            ans = max(ans, l_len + r_len);  // 两条链拼成路径（直径）
...
public:
    vector<string> generateParenthesis(int n) {
        ...
        auto dfs = [&](this auto&& dfs, int i, int balance)
        {
            ...
            {
                string path(n * 2, ')');
                for (int j : leftIndex)  path[j] = '('; // 左括号下标处改成 '('

                ans.emplace_back(path); // 收集
                return;
            }
...
public:
    int maxProfit(int k, vector<int>& prices) {
        ...
        // 初始化
        for (int j = 1; j < 2 * k; j += 2)	dp[0][j] = -prices[0]; // 奇数状态，持有

        for (int i = 1; i < prices.size(); i++)
        {
            for (int j = 0; j < 2 * k; j += 2) // 注意这里是j < 2*k，写2*k+1 会溢出，j = 2*k再+2，超出范围了
            {
                // 奇数j，持有
...
用一个**方格地图**，假如<span style="color:#CC0000;">每次搜索的方向为 **上下左右**</span>（不包含斜上方），那么给出一个**start起始位置**，那么BFS就是从四个方向走出第一步。
...
>
> 其实，我们仅仅<span style="color:#0000FF;">需要一个容器，能**保存遍历过的元素就可以**</span>，那么用**队列，还是用栈，甚至用数组**，都是可以的。
...
下面给出广搜代码模板（用队列），该模板针对的就是，上面的四方格的地图： （详细注释）
...
public:
    ...
    void dfs(const vector<vector<int>>& graph, int x) // x 当前遍历的节点值，值！值！是值！
    {
        // 终止  x == 3
        if (x == graph.size() - 1)  
        {
...
int main()
{
    ...
    {
        cin >> s >> t;
        graph[s].push_back(t); // 记录节点s有指向节点t的连接
    }
