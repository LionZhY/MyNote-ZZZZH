...
* **修饰成员变量**：**静态成员变量**
  * 所有类的对象，<span style="color:#0000CC;">**共享**同一个静态成员变量的副本，该变量**为所有对象所有**</span>
...
- delete [ ] arr; 释放arr指向的内存，**逐一调用数组中的每个对象的析构**
...
- 类对象数组
  ...
  - delete处理**数组类**类型的时候，会对每一个数组对象都调用它们的析构函数，然后再释放它们所占用的内存空间

  - delete只释放了指针指向的第一个对象的内存，只调用arr[0]的析构函数，造成内存泄露
...
只有当一个类被用来作为基类的时候，才把析构函数写成虚函数。**<span style="color:#0000FF;">基类的析构函数</span>一般建议声明为虚函数**。
...
- vector使用灵活的动态空间配置，维护一块连续的线性空间，在空间不足时，可以自动扩展空间容纳新元素，做到按需供给。

- 其在扩充空间的过程中仍然需要经历：
  - 重新配置空间，移动数据，释放原空间等操作
...
- 扩容规则
  ...
  - 新的更大的内存空间，<span style="color:#0000FF;">一般是当前空间的1.5倍或者2倍</span>，这个1.5或者2被称为扩容因子，不同系统实现扩容因子也不同。
...
- 词法分析 --- 语法分析 --- 语义分析 --- 生成中间代码 --- 优化 --- 生成目标代码![img](https://api2.mubu.com/v3/document_image/1d6a1d55-973b-4cfc-b3b6-30066dbb2ee2-915614.jpg)
